...
class JsonLogHandler(logging.Handler):
    def emit(self, record):
        json_out["logs"].append(self.format(record))
...
class DskSector:
    def __init__(self, raw_sector_header: bytes, file: BinaryIO, length: int):
        ...
        if (self.length and self.length != length) or self.size != length:
            raise DskFileException("Sector length found does not match sector length declared in track.")
...
class DskFile:
    def __init__(self, filepath: Path):
        ...
        if self.format[0:8] == b"MV - CPC":
            ...
            raise DskFileException("Only MV - CPCEMU or Extended DSK are supported.")
        ...
        if self.extended:
            ...
            self.tracks_sizes = [track_size for _ in range(self.tracks_nb * self.sides_nb)]
        ...
        for track_idx, (track_id, side_id) in enumerate(product(range(self.tracks_nb), range(self.sides_nb))):
            ...
            if declared_track_size:
                ...
                if actual_size != declared_track_size:
                    raise DskFileException(f"Track number {track_idx} has size {actual_size}, doesn't match declared {declared_track_size}")
                if track.side != side_id or track.number != track_id:
                    raise DskFileException(f"Found track number {track_idx} at offset {track.offset} with side id {track.side} and track id {track.number} when {side_id} and {track_id} are expected.")
                self.tracks.append(track)

    def __str__(self) -> str:
        return "{1} sided {2} disk image containing {0.tracks_nb} tracks per side ({3} tracks actually present)".format(
            self,
            {1: "One", 2: "Two"}[self.sides_nb],
            {True: "Extended", False: "CPCEMU"}[self.extended],
            len(self.tracks)
        )
        ...
        with target.open("wb") as cpr_file:
            ...
            for idx, chunk in enumerate(self.chunks):
                ...
                if len(chunk) < self.chunk_size:
                    ...
                    logging.debug(f"Padding chunk {ckid!r} by adding {padding_size} * '0xff'.")
...
class NoCartFile(CprFile):
    ...
    def _add_amsdos_chunk(self) -> None:
        ...
        if self.input_dsk.extended:
            logging.debug("Patching rom for extended disk format")
            patch = bytes([0x24, 0x00, 0x03, 0x07, 0x00, 0xfe, 0x00, 0x3f, 0x00, 0xc0, 0x00, 0x10, 0x00, 0x00, 0x00, 0xc1, 0x09, 0x2a, 0x52, 0xe5, 0x02, 0x04])
            amsdos_content[0x0A43:0x0A43 + len(patch)] = patch
            amsdos_content[0x056d] = 0x41
        else:
            min_sector_id = self.input_dsk.get_min_sector_id()
            logging.debug(f"CPC MV format. Min sector id is &{min_sector_id:02X}")
...
def check_cpr(args: argparse.Namespace) -> None:
    ...
    if not some_chunks_found:
        logging.warning("File is correct but contains no chunks.")
...
def _check_chunk(cpr_file: BinaryIO, chunk_idx: int):
    ...
    tag_str = chunk_tag.decode()
    json_out["chunks"].append({
        "index": chunk_idx,
        "offset_hex": f"0x{header_location:X}",
        "tag": tag_str,
        "size": chunk_size
    })
    ...
    if read_count != chunk_size:
        raise NoCartException(f"Chunk {chunk_idx:02} @ 0x{header_location:X} seems truncated. Read {read_count} of {chunk_size}.")
...
def create_cpr(args: argparse.Namespace) -> None:
    ...
    json_out["dsk_info"] = str(dsk_file)
    no_cart = NoCartFile(dsk_file, args.command, args.rompath)
    no_cart.write(args.output_file)

def interactive_prompt(args_list):
    """Fallback if missing args on an interactive terminal."""
    ...
    # Try to extract from args_list if provided
    if len(args_list) > 1 and args_list[1] in ['check', 'create', 'dumpdsk']:
        action = args_list[1]
    
    if len(args_list) > 2:
        input_file = args_list[2]
        
    if len(args_list) > 3:
        output_file = args_list[3]

    print("### Modo interactivo ROM (nocart)", file=sys.stderr)
    
    while action not in ['check', 'create', 'dumpdsk']:
        print("\n¿Qué acción deseas realizar? (check/create/dumpdsk): ", file=sys.stderr, end='')
        action = input().strip().lower()
        if action not in ['check', 'create', 'dumpdsk']:
            print("Acción inválida. Usa check, create o dumpdsk.", file=sys.stderr)
            
    while not input_file:
        print(f"\nArchivo de entrada para la acción '{action}' (.dsk o .cpr): ", file=sys.stderr, end='')
        temp = input().strip()
        if temp:
            input_file = temp
            
    if action in ['create', 'dumpdsk'] and not output_file:
        while not output_file:
            print(f"\nArchivo de salida (destino) para la acción '{action}': ", file=sys.stderr, end='')
            temp = input().strip()
            if temp:
                output_file = temp

    # Build the safe sys.argv
    new_args = [args_list[0], action, input_file]
    if output_file:
        new_args.append(output_file)
        
    # Preserve optional arguments like --command etc? We'll just append them at the end if they existed
...
def parse_args() -> argparse.Namespace:
    ...
    args_list = sys.argv
    try:
        # Try parse normally, if it fails and interactive, catch it
        if sys.stdin.isatty() and (len(args_list) < 3 or (len(args_list) < 4 and args_list[1] in ['create', 'dumpdsk'])):
            args_list = interactive_prompt(args_list)
        return parser.parse_args(args_list[1:])
    except SystemExit:
        if sys.stdin.isatty():
             # Last resort if user inputs were somehow wrong
             print("\n### Error en los argumentos. Intenta de nuevo interactivo.", file=sys.stderr)
             args_list = interactive_prompt([sys.argv[0]])
             return parser.parse_args(args_list[1:])
        raise
...
def main():
    try:
        ...
        print(json.dumps({"status": "error", "error": f"Invalid arguments. Check usage."}))
        sys.exit(syserror.code)
    ...
    json_out["input_file"] = str(args.input_file)
    if args.output_file:
         json_out["output_file"] = str(args.output_file)
    ...
    try:
        ...
        json_out["status"] = "error"
        json_out["error"] = f"File not found: {exc}"
        print(json.dumps(json_out, indent=2))
        sys.exit(1)
