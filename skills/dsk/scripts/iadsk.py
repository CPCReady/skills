...
class DiskHeader:
    ...
    def check(self, tracks, sztrack, sides):
        ...
        if self.sides != sides:
            raise FormatError(
                "header number of sides (%d) differs from expected values (%d)"
                % (self.sides, sides)
            )
        if self.tracks < tracks:
            raise FormatError(
                "header number of tracks (%d) differs from expected value (%d)"
                % (self.tracks, tracks)
            )
        if self.sztrack != sztrack:
            raise FormatError(
                "header track size (%d) differs from expected value (%d)"
                % (self.tracks, tracks)
            )
...
class TrackSectorInfo:
    ...
    def check(self, track):
        if self.C != track:
            raise FormatError(
                "unexpected sector info track number (%d), %d was expected"
                % (self.C, track)
            )
        if self.N != 2:
            raise FormatError(
                "sector info N value (%s) differs from expected value (0x02)"
                % (hex(self.N))
            )
        if self.H != 0:
            raise FormatError(
                "sector info H value (%s) differs from expected value (0x00)"
                % (hex(self.H))
            )
        if self.R < CPM_MIN_SECTOR or self.R > CPM_MAX_SECTOR:
            raise FormatError(
                "sector R value (%s) differs from expected value C1-C9" % (hex(self.R))
            )
...
class TrackHeader:
    ...
    def __init__(self, track, sectors, basetrack, side):
        ...
        while addedsectors < sectors:
            ...
            if addedsectors < self.sectors:
                self.sectors_info.append(
                    TrackSectorInfo(sec_second, track, side, basetrack)
                )
    ...
    def set(self, sectors, content):
        ...
        if self.sectors != sectors:
            raise FormatError(
                "expected number of sectors per track is 9, not %d" % (self.sectors)
            )
    ...
    def check(self):
        ...
        if self.side != 0:
            raise FormatError(
                "track side number should be 0, %d was found" % (self.side)
            )
        if self.sectors != DEF_SECTORS:
            raise FormatError(
                "track number of sectors (%d) differes from expected value (%d)"
                % (self.sectors, DEF_SECTORS)
            )
        if self.szsector != 2:
            raise FormatError(
                "track sector size code (%d) differs from expected value (2)"
                % (self.szsector)
            )
        if self.gap3 != 78:
            raise FormatError(
                "track GAP#3 field value (%d) differs from expected value (78)"
                % (self.gap3)
            )
        if self.filler != CPM_DELETED:
            raise FormatError(
                "track FILL field value (%d) differs from expected value (%d)"
                % (self.filler, CPM_DELETED)
            )
        for i in range(0, DEF_SECTORS):
...
class TrackData:
    ...
    def __init__(self, sectors, filler):
        self.data = bytearray()
        for i in range(0, 512 * sectors):
            self.data.extend(filler.to_bytes(1, "little"))
        self.sectors = sectors
    ...
    def set(self, sectors, content):
        ...
        if len(content) < datasz:
            raise FormatError(
                "track size area is less than 512 bytes x %d sectors", sectors
            )
    ...
    def check(self):
        ...
        if len(self.data) != datasz:
            raise FormatError(
                "track data size (%d) differes from expected value (%d)"
                % (len(self.data, datasz))
            )
...
class Disk:
    def __init__(
        ...
        self.tracks = [
            Track(i, self.nsectors, CPM_MIN_SECTOR) for i in range(0, self.ntracks)
        ]
    ...
    def set(self, content):
        ...
        if self.ntracks < len(self.tracks):
            raise FormatError(
                "unexpected number of tracks (%d vs %d)"
    ...
    def check(self):
        if self.ntracks < len(self.tracks):
            raise FormatError(
                "number of tracks (%d) differs from expected values (%d)"
                % (self.ntracks, len(self.tracks))
            )
...
class DirEntry:
    ...
    def __init__(self, num):
        ...
        self.extend = (
            CPM_DELETED  # 0-31 (large files can spread through several entries)
        )
    ...
    def to_sectors(self, iblock, npages=0):
        ...
        if npages == 0:
            npages = self.pages
            for i in range(0, iblock):
                npages = npages - CPM_CLUSTER_PAGES
    ...
    def dump(self):
        ...
        if self.status == CPM_DELETED:
            ...
            print(
                self.name.decode("utf-8") + "." + self.ext.decode("utf-8"), end="  [ "
            )
            print(
                "st:",
                self.status,
...
class DirTable:
    ...
    def can_allocate(self, filebytes):
        ...
        """
        numentries = math.ceil(
            filebytes / (16 * 1024)
        )  # each entry points to 16K of data
        freeentries = 0
        startentry = 0
    ...
    def write_entries(self, ientry, filename, fbytes):
        ...
        fext = (
            bytearray(b"\x20\x20\x20")
            if len(filecomp) == 1
            else bytearray(filecomp[1][0:3].upper().encode("utf-8"))
        )
        ...
        while filepages > 0:
            ...
            for ec in range(0, 16):
                if ec < eclusters:
                    ...
                    for s in range(
                        cluster * CPM_CLUSTER_SECTORS,
                        (cluster + 1) * CPM_CLUSTER_SECTORS,
                    ):
                        file_sectors.append(
                            (math.floor(s / DEF_SECTORS), s % DEF_SECTORS)
                        )
                else:
                    e.clusters[ec] = 0x00
            e.extend = extend
...
class AmsdosHead:
    ...
    def __init__(self):
        ...
        self.file_name = bytearray(
            0x20 for i in range(0, 8)
        )  # unused chars filled with spaces
        self.file_ext = bytearray(
            0x20 for i in range(0, 3)
        )  # unused chars filled with spaces
        ...
        self.custom = bytearray(
            0x00 for i in range(0, 36)
        )  # unused but affects checksum
    ...
    def dump(self):
        ...
        print(
            f"  File: {self.file_name.decode('utf-8') + '.' + self.file_ext.decode('utf-8')}"
        )
        print(
            f"  File type: {self.file_type} File size: {self.file_size} User ID: {self.user}"
        )
        print(
            f"  Load address: {hex(self.addr_load)} Exec address: {hex(self.addr_entry)}"
        )
...
def find_entry_by_name(dirtable, filename):
    """
    Finds directory entry by filename (case-insensitive).
    Returns entry or None.
    """
    filename_upper = filename.upper()
    for e in dirtable.entries:
        if e.status != CPM_DELETED and e.extend == 0:
            if e.get_filename().upper() == filename_upper:
                return e
    return None


def run_free(args, disk):
    """
    Shows disk space usage:
    - Total capacity
    - Free space (KB and clusters)
    - Used space
    - Cluster map visualization
    """
    run_check(args, disk)
    dirtable = disk.get_dirtable()
    clusters, free_kb = dirtable.get_disk_clusters()

    total_kb = int((DEF_SECTORS * DEF_TRACKS * 512) / 1024)
    used_kb = total_kb - free_kb
    total_clusters = len(clusters)
    free_clusters = sum(clusters)
    used_clusters = total_clusters - free_clusters
    percent_free = round((free_kb / total_kb) * 100, 1)
    ...
    print(f"\n| Metric         | Value    |")
    print(f"|----------------|----------|")
    print(f"| Total Capacity | {total_kb:>6} KB |")
    print(f"| Free Space     | {free_kb:>6} KB |")
    print(f"| Used Space     | {used_kb:>6} KB |")
    print(f"| Free %         | {percent_free:>6} % |")
    print(f"| Used %         | {percent_used:>6} % |")
    print(f"\n| Clusters       | Count    |")
    print(f"|----------------|----------|")
    print(f"| Free Clusters  | {free_clusters:>6}   |")
    print(f"| Used Clusters  | {used_clusters:>6}   |")
    print(f"| Total Clusters | {total_clusters:>6}   |")

    print(f"\n**Cluster Map** (first {min(32, total_clusters)} clusters):")
    map_str = ""
    for i, c in enumerate(clusters[:32]):
        map_str += "▫" if c else "▪"
        if (i + 1) % 8 == 0:
            map_str += " "
    print(f"```\n{map_str}\n```")
...
def run_era(args, disk):
    """
    Deletes a file from disk by name.
    Handles multi-entry files (>16K).
    """
    run_check(args, disk)
    dirtable = disk.get_dirtable()
    filename = args.era.upper()

    entry = find_entry_by_name(dirtable, filename)
    if entry is None:
        print(f"[iadsk] error: archivo no encontrado '{filename}'")
        sys.exit(1)

    ientry = dirtable.entries.index(entry)
    sectors, npages = dirtable.to_file_sectors(ientry)
    freed_kb = math.ceil(npages * CPM_PAGE_BYTES / 1024)
    filename_str = entry.get_filename()

    entry.status = CPM_DELETED

    if entry.extend > 0:
        ext_entry = dirtable.entries[ientry + 1]
        while ext_entry.status != CPM_DELETED and ext_entry.extend > 0 and ientry < 62:
            ext_entry.status = CPM_DELETED
            ientry += 1
            ext_entry = dirtable.entries[ientry + 1]
    ...
    print("## File Deletion")
    print(f"\n**Archivo eliminado:** `{filename_str}`")
    print(f"**Espacio liberado:** {freed_kb} KB")


def run_list(args, disk):
    """
    Lists content of a file by name.
    Supports text and hex dump modes.
    """
    run_check(args, disk)
    dirtable = disk.get_dirtable()

    entry = find_entry_by_name(dirtable, args.list.upper())
    if entry is None:
        print(f"[iadsk] error: archivo no encontrado '{args.list}'")
        sys.exit(1)
    ...
    has_header = False
    if len(data) >= 128:
        header.set(data[0:128])
        has_header = header.is_valid_header()
        if has_header:
            data = data[128:]
            real_size = header.file_size
            data = data[:real_size]

    print(f"## File Content: {filename}")
    print(f"\n**Tamano:** {len(data)} bytes")
    print(f"**Header AMSDOS:** {'Si' if has_header else 'No'}")
    ...
    if args.hex:
        ...
        try:
            ...
            ext = entry.ext.decode("utf-8").strip().lower()
            lang = "basic" if ext == "bas" else "text"
            print(f"\n```{lang}")
            print(text.rstrip("\r\n"))
            print("```")
        except Exception:
            print(f"\n[iadsk] warning: no se puede mostrar como texto")
            print(f"[iadsk] usa --hex para ver como hex dump")
...
def run_put_binfile(args, disk, infile):
    ...
    if args.start_addr != None:
        header.addr_entry = run_get_start(args.start_addr, mapfile)
...
def process_args():
    ...
    parser.add_argument(
        "dskfile",
        help="DSK file. Used as input/output depending on the arguments used.",
    )
    parser.add_argument(
        "-n", "--new", action="store_true", help="Creates a new empty DSK file."
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="Checks if the DSK file format is compatible.",
    )
    parser.add_argument(
        "--dump",
        action="store_true",
        help="Prints DSK file format information on the standard output.",
    )
    parser.add_argument(
        "--cat",
        action="store_true",
        help="Lists in the standard output the DSK file content.",
    )
    parser.add_argument(
        "--free",
        action="store_true",
        help="Shows free space on disk (KB, clusters, percentage).",
    )
    parser.add_argument(
        "--era",
        type=str,
        help="Deletes a file by name (e.g., FILE.BAS).",
    )
    parser.add_argument(
        "--list",
        type=str,
        help="Lists file content by name.",
    )
    parser.add_argument(
        "--hex",
        action="store_true",
        help="Show file content as hex dump (with --list).",
    )
    parser.add_argument(
        "--header",
        type=int,
        help="Prints AMSDOS header for indicated file entry starting at 0.",
    )
    parser.add_argument(
        "--no-header",
        action="store_true",
        help="Used together with the --get option extracts files without AMSDOS header.",
    )
    parser.add_argument(
        "--get",
        type=int,
        help="Extracts file pointed by the indicated entry. Use --no-header to remove AMSDOS header.",
    )
    parser.add_argument(
        "--put-bin",
        type=str,
        help="Adds a new binary file to DSK file generating an AMSDOS header.",
    )
    parser.add_argument(
        "--put-raw",
        type=str,
        help="Adds a new binary file to DSK file without generating an AMSDOS header.",
    )
    parser.add_argument(
        "--put-ascii",
        type=str,
        help="Adds a new ASCII file to DSK file. The file should not include an AMSDOS header.",
    )
    parser.add_argument(
        "--map-file",
        type=str,
        help="Imports a map file with symbol names and addresses that can be referenced in the --start-addr option",
    )
    parser.add_argument(
        "--load-addr",
        type=aux_int,
        default=0x4000,
        help="Initial address to load the file (default 0x4000). Only used in binary files with appended AMSDOS headers.",
    )
    parser.add_argument(
        "--start-addr",
        type=str,
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f" IADSK Tool Version {__version__}",
...
def main():
    ...
    if args.era:
        run_era(args, disk)
    if args.header != None:
        run_dump_header(args, disk)
    if args.get != None:
        run_get_file(args, disk)
    if args.list:
        run_list(args, disk)
    if args.put_ascii != None:
        run_put_asciifile(args, disk)
    if args.put_bin != None:
        run_put_binfile(args, disk, args.put_bin)
    if args.put_raw != None:
        run_put_rawfile(args, disk, args.put_raw)
    sys.exit(0)
